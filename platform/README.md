...
20 条里已有 4 条顺手验过（1、2、10、17），全部是 Task 13/17 跑 `mdm/customer` 时人工核对的，还没固化成可重跑的自动化测试——那是档 1 加满 5 个组件之后的事。用例 4 单独标了"档 1 验"：`mdm/customer` 自己没有依赖方，这条断言的对象（依赖方容器里的 `MDM_CUSTOMER_GRPC_ENDPOINT`）现在根本不存在验证的前提。其余 15 条仍是清单。
...
| 2 | `assembly.yaml` 靠 `artifacts` 随组件分发 | 声明 `type: metadata`，`up` 后检查 `.brickkit/artifacts/<服务名>/metadata/assembly.yaml` 在 | §3.5 | ✅ 已验证（Task 17，`mdm/customer`）：`ls .brickkit/artifacts/mdm-customer-1-0-0/metadata/assembly.yaml` 存在 |
| 3 | 地址变量名由 ID 推导、不带版本号 | 断言 `MDM_CUSTOMER_ENDPOINT=http://mdm-customer-1-0-0:8080` | §2.1、§3.4 | ⏳ 待实现 |
| 4 | 额外端口地址是 `http://` 而不是 `grpc://` | 断言 `MDM_CUSTOMER_GRPC_ENDPOINT` 的值以 `http://` 开头，且组件的 gRPC 客户端剥掉 scheme 后能连上 | §2.1 | ⏳ 档 1 验——`mdm/customer` 目前没有任何依赖方，容器里根本不会有 `MDM_CUSTOMER_GRPC_ENDPOINT` 这个变量（这是对的，见 Task 17）。要等档 1 有了 `erp-sales` 才能在**依赖方**容器里断言这个变量存在且以 `http://` 开头。不许当成已验过 |
...
| 10 | 默认启动宽限是 60 秒 | 造一个冷启动 45 秒的组件，不写 `startPeriodSeconds`，断言它能起来 | §12.3.5 | ✅ 部分验证（Task 17，`mdm/customer`）：`docker inspect ... .Config.Healthcheck.StartPeriod` 确实是 `60000000000`（60 秒的纳秒数）。**只验了配置值，没有专门造一个 45 秒冷启动的组件去验"能不能撑住"**——mdm-customer 冷启动远快于 45 秒，这条边界情形留给后面某个真的启动慢的组件顺手验 |
...
| 17 | `up` 只打印建库语句，不建库 | 断言输出里有 `CREATE DATABASE`，而库并没有被创建 | §2.7.2 | ✅ 已验证（Task 17，`mdm/customer`）：`brickkit up 2>&1 \| grep -i 'CREATE DATABASE'` 命中 |
