...
## 现状（阶段二 Task 19-20，20 条全部落地）

六组全部固化成 `go test` 自动化测试（`a`/`b`/`c`/`d`/`e`/`f`_test.go），`make tier1` 一条命令跑完，**20 条全绿**。

- **A/D/E 三组**（Task 19，11 条）：每条测试自己在 `t.TempDir()` 里现搭一个最小 `brickkit.yaml` + 假 `component.yaml`，不依赖装配仓库本身的状态、也不产生任何副作用。
- **B 组**（Task 20，用例 3/4/5/8）：用例 3/4/5 直接对着 `brickkit up` 真实起的五组件部署（Task 18）做 `docker exec` 断言——`erp-sales` 是全阶段二唯一有真实依赖方的组件，这三条阶段一根本没有验证前提。用例 8 是"`enabled: false` 级联"这条通用机制，用隔离 fixture 验，不扰动真实部署。
