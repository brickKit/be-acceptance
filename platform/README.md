...
单一、连续、可追溯的清单，不是把它们并进 23 条里。用例 26-27 又绕回了
tier1（brickKit 自身的多版本共存机制，`servedBy` 协作验证中顺带发现的，
见文末对应一节），所以编号"连续"但"档位"不连续，读的时候留意每条自己
标的档位，不要单看编号顺序猜。

## 现状（阶段二 Task 19-20 的 20 条 + 阶段三 Task 15 补的 3 条 + `servedBy` 协作验证顺带补的 2 条，共 25 条全部落地）

`a`~`h`_test.go 固化成 `go test` 自动化测试，`make tier1` 一条命令跑完，**25 条全绿**。
...
## tier1 补充：`servedBy` 协作验证中顺带确认的两条 brickKit 自身机制（用例 26-27）
...
| # | 验什么 | 怎么验 | 对应本书 | 状态 |
|---|---|---|---|---|
| 26 | 同一个组件 ID 的两个精确版本可以同时独立部署，各自依赖方拿到正确指向"自己那个版本"的地址 | 两个独立 local 源（一个源目录物理上只能服务一个版本，双源各自放一个版本）+ 两个依赖方各自声明依赖不同精确版本 | "精确版本+版本化服务名"机制，四条铁律之一的自然延伸 | ✅ `TestPlatform26_同一个组件ID两个精确版本同时声明地址互不干扰`：一次没命中的尝试——最初把两个版本的 `component.yaml` 塞进同一个源目录覆盖着写，直接报 `COMPONENT_NOT_FOUND`（一个 local 源目录物理上只服务一个版本），改成两个独立源目录后才对 |
| 27 | 同一个 ID 同一个精确版本重复声明，直接拒绝 | 两条 `components:` 条目写完全相同的 `id@version` | brickKit 自己在 `servedBy` 请求文档里的预判（既有校验，非 `servedBy` 新加规则） | ✅ `TestPlatform27_同id同精确版本重复声明直接拒绝`：`CONFIG_INVALID`，"重复声明了 foo/bar@1.0.0" |

`h_test.go` 承载这两条。`servedBy` 本身相关的 5 条静态校验（自引用/链式
